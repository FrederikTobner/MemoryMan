...
#define GENERATE_TABLE_FUNCTIONS(TYPE, HASH_FUNCTION)                                                            \
    ...
    TYPE##_table_t * TYPE##_table_new() {                                                                        \
        TYPE##_table_t * table = untraced_malloc(sizeof(TYPE##_table_t));                                        \
        if (!table) {                                                                                            \
            return NULL;                                                                                         \
        }                                                                                                        \
    ...
    TYPE##_table_entry_t * TYPE##_table_entry_new(TYPE address, char const * key) {                              \
        TYPE##_table_entry_t * entry = untraced_malloc(sizeof(TYPE##_table_entry_t));                            \
        if (!entry) {                                                                                            \
            return NULL;                                                                                         \
        }                                                                                                        \
        entry->data = address;                                                                                   \
    ...
    int TYPE##_table_init_table(TYPE##_table_t * table) {                                                        \
        ...
        table->entries = untraced_malloc(table->allocated * sizeof(TYPE##_table_entry_t));                       \
        if (!table->entries) {                                                                                   \
            return -1;                                                                                           \
        }                                                                                                        \
