#ifndef TABLE_CODE_GENERATION_H_
#define TABLE_CODE_GENERATION_H_
...
#define CHECKED_MALLOC_USING_TYPE(n, type)                                     \
  (SIZE_CHECK_USING_TYPE((n), (type)) ? malloc((n) * sizeof(type)) : 0)
...
#define GENERATE_TABLE_PROTOTYPES(TYPE)                                        \
  TYPE##_table_entry_t *TYPE##_table_entry_new(TYPE opcodeAddress,             \
                                               char const *key);               \
  ...
  int TYPE##_table_insert_entry(TYPE##_table_entry_t *entry,                   \
                                TYPE##_table_t *table);                        \
  TYPE##_table_entry_t *TYPE##_table_remove_entry(TYPE##_table_entry_t *entry, \
                                                  TYPE##_table_t *table);      \
  TYPE##_table_entry_t *TYPE##_table_look_up_entry(char const *key,            \
                                                   TYPE##_table_t *table);
...
#define GENERATE_TABLE_FUNCTIONS(TYPE, HASH_FUNCTION)                          \
  ...
  TYPE##_table_t *TYPE##_table_new() {                                         \
    TYPE##_table_t *table = malloc(sizeof(TYPE##_table_t));                    \
    if (!table) {                                                              \
      return NULL;                                                             \
    }                                                                          \
  ...
  TYPE##_table_entry_t *TYPE##_table_entry_new(TYPE address,                   \
                                               char const *key) {              \
    TYPE##_table_entry_t *entry = malloc(sizeof(TYPE##_table_entry_t));        \
    if (!entry) {                                                              \
      return NULL;                                                             \
    }                                                                          \
    entry->data = address;                                                     \
  ...
  void TYPE##_table_free_entries(TYPE##_table_t *table) {                      \
    ...
    for (size_t i = 0; i < table->allocated; i++) {                            \
      if (!table->entries[i] ||                                                \
          table->entries[i] ==                                                 \
              (TYPE##_table_entry_t *)(0xFFFFFFFFFFFFFFFFUL)) {                \
        break;                                                                 \
      }                                                                        \
  ...
  int TYPE##_table_insert_entry(TYPE##_table_entry_t *node,                    \
                                TYPE##_table_t *table) {                       \
    if (!node || !table) {                                                     \
      return -1;                                                               \
    }                                                                          \
    if (table->used >=                                                         \
        ((double)table->allocated) * TABLE_GROWTH_TRIGGER_VALUE) {             \
  ...
  TYPE##_table_entry_t *TYPE##_table_remove_entry(TYPE##_table_entry_t *node,  \
                                                  TYPE##_table_t *table) {     \
    if (!table) {                                                              \
      return NULL;                                                             \
    }                                                                          \
  ...
  static int TYPE##_table_grow_table(TYPE##_table_t *table) {                  \
    TYPE##_table_entry_t **newEntries = CHECKED_MALLOC_USING_TYPE(             \
        table->allocated * GROWTH_FACTOR, *table->entries);                    \
    if (!newEntries) {                                                         \
      return -1;                                                               \
    }                                                                          \
